...
class Pawn: Piece {
    ...
    func getMove(board: [[Piece]], rank: Int, file: Int) -> [(rank: Int, file: Int)] {
        ...
        if Util.common.isValidCoordinate(coordinate: (rank+direction, file)) && board[rank+direction][file] is Empty {
            moves.append((rank+direction, file))
            if rank == 6-color*5 && board[rank+2*direction][file] is Empty {
                moves.append((rank+2*direction, file))
            }
        }
        if Util.common.isValidCoordinate(coordinate: (rank+direction, file-1)) && !(board[rank+direction][file-1] is Empty) && board[rank+direction][file-1].color != color {
